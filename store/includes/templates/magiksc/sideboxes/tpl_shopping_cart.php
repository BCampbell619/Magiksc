  ...
  $content .= '<div id="cartBoxListWrapper">' . "\n" . '<ul class="list-group">' . "\n";
    ...
    for ($i=0, $n=sizeof($products); $i<$n; $i++) {
      ...
      if (isset($_SESSION['new_products_id_in_cart']) && ($_SESSION['new_products_id_in_cart'] == $products[$i]['id'])) {
        ...
        $content .= '<span class="text-dark">';
      }

      $content .= '</span><a class="text-dark" href="' . zen_href_link(zen_get_info_page($products[$i]['id']), 'products_id=' . $products[$i]['id']) . '">';
      ...
      $content .= $products[$i]['name'] . '</span></a> (' . $products[$i]['quantity'] . ')</li>' . "\n";
