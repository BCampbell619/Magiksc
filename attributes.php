...
/* 1 of 2
/****************************************************/
// 23997 is the ID for the GRID option
if (!defined('CONFIG_ATTRIBUTE_OPTION_GRID_INSTALLED') || CONFIG_ATTRIBUTE_OPTION_GRID_INSTALLED != 'true') {
  $check_install_rs = $db->Execute("select products_options_types_name from ".TABLE_PRODUCTS_OPTIONS_TYPES." where products_options_types_id = '23997'");
  if ($check_install_rs->EOF) {
    zen_db_perform(TABLE_PRODUCTS_OPTIONS_TYPES, array('products_options_types_name' => 'Grid', 'products_options_types_id' => '23997'));
  }
  if (defined('CONFIG_ATTRIBUTE_OPTION_GRID_INSTALLED')) {
	zen_db_perform(TABLE_CONFIGURATION, array('configuration_value' => 'true', 'last_modified' => 'now()'), 'update', "configuration_key = 'CONFIG_ATTRIBUTE_OPTION_GRID_INSTALLED'");
  } else {
	zen_db_perform(TABLE_CONFIGURATION, array('configuration_title' => 'Attribute Grid Installed', 'configuration_key' => 'CONFIG_ATTRIBUTE_OPTION_GRID_INSTALLED', 'configuration_value' => 'true', 'date_added' => 'now()'));
  }
}
$check_attributes_sql 		= "select pa.options_id, po.products_options_type ".
...
if ($check_grid) {
  ...
  $sql = "select distinct popt.products_options_id, popt.products_options_name, popt.products_options_sort_order, popt.products_options_comment
				  ...
				  and 		popt.products_options_type = '23997'
				  and		popt.language_id = '" . (int)$_SESSION['languages_id'] . "' " .
  $options_order_by;
  $products_options_names = $db->Execute($sql);
  $show_only_grid = ($check_attributes_count == $products_options_names->RecordCount());
  for($d=0; $d < 2; $d++) {
	$rd = ($rd != 'H' ? 'H' : 'V');
	if ($products_options_names->RecordCount() == $d) {
	  ...
	  $grid_records[$rd] = array(	'id'		=>	$products_options_names->fields['products_options_id'],
									'name'		=>	$products_options_names->fields['products_options_name'],
									'comment'	=>	$products_options_names->fields['products_options_comment'],
									'options'	=>	array(),
									'images'	=>	false
								  );
	  ...
	  while (!$products_options->EOF) {
	    ...
	    if (zen_not_null($products_options->fields['attributes_image'])) {
...
RMOVE THIS CODE AS REPLICATED WITH MORE DETAIL ABOVE
              ...
              and       pov.language_id = '" . (int)$_SESSION['languages_id'] . "' " .
                ...
                while (!$products_options->EOF) {
                  ...
                  if (($flag_show_weight_attrib_for_this_prod_type=='1' and $products_options->fields['products_attributes_weight'] != '0')) {
                    $products_options_display_weight = ATTRIBUTES_PRICE_DELIMITER_PREFIX . $products_options->fields['products_attributes_weight_prefix'] . round($products_options->fields['products_attributes_weight'],2) . TEXT_PRODUCT_WEIGHT_UNIT . ATTRIBUTES_WEIGHT_DELIMITER_SUFFIX;
...
/* 3 of 3
/****************************************************/
				  case ($products_options_names->fields['products_options_type'] == '23997'): // GRID LAYOUT
